...
for simulation in range(simul):
    ...
    while True:
        ...
        while True:
            ...
            print(arbi)
            print(arbi.T.numpy() - np.concatenate((A, B), axis=1))
...
np.savetxt('3D-gaussian_mixture-regret.csv', cumreg_ULA.T, delimiter=',')
np.savetxt('3D-gaussian_mixture-moderr.csv', model_error.T, delimiter=',')
np.savetxt('3D-gaussian_mixture-rejection.csv', rejection.T, delimiter=',')
np.savetxt('3D-gaussian_mixture-time.csv', time_simul.T, delimiter=',')
for simulation in range(simul):
    np.savetxt(f'3D-gaussian_mixture-traj{simulation + record}.csv', traj[simulation], delimiter=',')
...
plt.plot(x_vals, y, 'r-', label='Symmetric GM 3D')
