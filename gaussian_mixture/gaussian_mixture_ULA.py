...
def gaussian_mixture_ULA(mean, r, lam, phi_U, data, n, m, t_k, device):
    ...
    for step in range(step_iter):
        ...
        gaussian_mixture_grad_log(phi_gpu, z_batch, x_prime_batch, mean_gpu, grad_log_b, grad_log_phi, grad_U, w,
        ...
        # t6 = time.time()
        # if (step % 100000 == 0):
        #     print(f'iteration {step + 1:7d}: {t6 - t1:.3e}s')
        #     print(f'time detail: {t2 - t1:.3e}, {t3 - t2:.3e}, {t4 - t3:.3e}, {t5 - t4:.3e}, {t6 - t5:.3e}')
