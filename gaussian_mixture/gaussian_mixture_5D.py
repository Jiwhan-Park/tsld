...
# Noise parameters
gm_a = np.array([1/4, 1/4, 1/4, 1/4, 1/4])
r = 1
...
np.savetxt('5D-gaussian_mixture-regret.csv', cumreg_ULA.T, delimiter=',')
np.savetxt('5D-gaussian_mixture-moderr.csv', model_error.T, delimiter=',')
np.savetxt('5D-gaussian_mixture-rejection.csv', rejection.T, delimiter=',')
np.savetxt('5D-gaussian_mixture-time.csv', time_simul.T, delimiter=',')
for simulation in range(simul):
    np.savetxt(f'5D-gaussian_mixture-traj{simulation + record}.csv', traj[simulation], delimiter=',')
...
plt.plot(x_vals, y, 'r-', label='Symmetric GM 5D')
