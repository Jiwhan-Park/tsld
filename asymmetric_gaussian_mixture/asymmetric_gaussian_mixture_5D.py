...
from asymmetric_gaussian_mixture_ULA import *
from asymmetric_gaussian_mixture_ULA import *
...
# System matrices
A = torch.tensor([[0.3, 0.6, 0.2, 0.3, 0.1],
                  [0.0, 0.1, 0.4, 0.0, 0.6],
                  [0.1, 0.5, 0.3, 0.0, 0.2],
                  [0.4, 0.0, 0.3, 0.3, 0.0],
                  [0.3, 0.3, 0.1, 0.4, 0.4]])
B = torch.tensor([[0.5, 0.4, 0.2, 0.5, 0.4],
                  [0.6, 0.0, 0.3, 0.1, 0.3],
                  [0.5, 0.0, 0.0, 0.1, 0.2],
                  [0.1, 0.5, 0.0, 0.2, 0.4],
                  [0.2, 0.1, 0.6, 0.0, 0.0]])
...
gm_a = np.array([1/2, 1/2, 1/2, 1/2, 1/2])
r = 1
gamma = 1 / 4
eta = math.exp((1 - 2 * gamma) * np.linalg.norm(gm_a)**2 / 2)
mu = np.array([gamma * gm_a, -(1 - gamma) * gm_a])
cov = np.array([r * np.eye(n), r * np.eye(n)])
weights = np.array([1 - gamma, gamma])
W = r * np.eye(n) + gamma * (1 - gamma) * gm_a[:, None] @ gm_a[:, None].T
...
for simulation in range(simul):
    ...
    while True:
        ...
        phi_U = newton_method(data, gm_a, r, n, m, lam, gamma, eta, 1000, device) # ((n + m) * n, 1)

        # Policy optimization with rejection sampling
        while True:
            start = time.time()
            arbi = asymmetric_gaussian_mixture_ULA(gm_a, r, lam, gamma, eta, phi_U, data, n, m, t_k, device)
            end = time.time()
        ...
        if t == time_horizon:
            ...
            break
    simend = time.time()
    time_simul[simulation] = simend - simstart

# Save and plot results
cumreg_ULA = np.cumsum(regret_ULA, axis=1)
np.savetxt('5D-asymmetric_gaussian_mixture-regret.csv', cumreg_ULA.T, delimiter=',')
np.savetxt('5D-asymmetric_gaussian_mixture-moderr.csv', model_error.T, delimiter=',')
np.savetxt('5D-asymmetric_gaussian_mixture-rejection.csv', rejection.T, delimiter=',')
np.savetxt('5D-asymmetric_gaussian_mixture-time.csv', time_simul.T, delimiter=',')
for simulation in range(simul):
    np.savetxt(f'5D-asymmetric_gaussian_mixture-traj{simulation + record}.csv', traj[simulation], delimiter=',')
...
plt.plot(x_vals, y, 'r-', label='Asymmetric GM 5D')
